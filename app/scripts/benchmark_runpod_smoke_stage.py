...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Mirror the canonical SingleRaw smoke sample into a zip-excluded RunPod upload bundle."
    )
    parser.add_argument("--output-dir", required=True, help="Benchmark output directory that should receive the smoke stage artifact.")
    ...
    parser.add_argument(
        "--write-canonical",
        action="store_true",
        help="Write the canonical rawprep_runpod_smoke_stage.json artifact and the smoke-input bundle.",
    )
    parser.add_argument(
        "--skip-archive",
        action="store_true",
        help="Write only the staged sample tree without creating rawprep_runpod_smoke_sample_bundle.zip.",
    )
    ...
    request = RawPrepBenchmarkRunPodSmokeStageRequest(
        ...
        write_archive=not args.skip_archive,
    )
    stage = (
        write_rawprep_benchmark_runpod_smoke_stage(request)
        if args.write_canonical
        else build_rawprep_benchmark_runpod_smoke_stage(request)
    )
    payload = json.dumps(stage.model_dump(), indent=2, ensure_ascii=False)
    if args.out:
