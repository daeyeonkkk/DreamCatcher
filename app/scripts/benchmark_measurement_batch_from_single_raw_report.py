...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Write measured benchmark result JSON for multiple official benchmark samples by bridging SingleRaw report timing artifacts from a JSON or JSONL batch file."
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Path to a JSON/JSONL file containing batch entries with sample_id and report_path.",
    )
    parser.add_argument("--output-root", default="outputs", help="Output root used for benchmark artifacts.")
    parser.add_argument("--out", help="Optional JSON output path for the batch bridge summary.")
    args = parser.parse_args()

    batch = write_rawprep_benchmark_measurement_batch_from_single_raw_report(
        RawPrepBenchmarkMeasurementFromSingleRawReportBatchRequest(
            output_root=args.output_root,
            entries=load_rawprep_benchmark_measurement_from_single_raw_report_batch_entries(args.input),
        )
    )
